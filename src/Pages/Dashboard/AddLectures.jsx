...
function AddCourseLectures() {
  ...
  async function onFormSubmit(e) {
    ...
    if (
      ...
      (!userInput.lecture && !userInput.videoSrc)
    ) {
      toast.error('Tất cả các trường đều bắt buộc');
      return;
    }
    const response = await dispatch(
      addCourseLectures({ userInput, uploadType })
    );
      ...
      <div className="min-h-[90vh] text-white flex flex-col items-center justify-center gap-10 mx-5 sm:mx-16 md:mx-20">
        <div className="flex flex-col gap-5 p-2 shadow-[0_0_10px_black] w-[80vw] md:w-96 rounded-lg">
          ...
          <form onSubmit={onFormSubmit} className="flex flex-col gap-3">
            ...
            {/* Hiển thị input tùy theo loại upload */}
            {uploadType === 'upload' ? (
              ...
              <>
                <input
                  ...
                  className="bg-transparent px-3 py-1 border"
                  value={userInput.videoSrc}
                />
                <input
                  type="text"
                  name="duration"
                  placeholder="Nhập thời lượng video mm:ss"
                  onChange={handleInputChange}
                  className="bg-transparent px-3 py-1 border"
